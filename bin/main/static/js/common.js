...
let removeCart = (event) => {
	let removeTarget = $(event.target).parent().parent();
	let tdList = $(removeTarget).find('td');
	
	let targetId = $(tdList[0]).text();
	
	cartList = cartList.filter((cart) => {
		return cart.id !== targetId;
	});
	...
	$.ajax({
		...
		historyList.forEach((history, index) => {
			$(tbody).append($('<tr/>').append(
					$('<td/>', {'text': history.goodsName}),
					$('<td/>', {'text': history.itemCount}),
					$('<td/>', {'text': history.createdAt})
			));
		});
		$('#history').dialog('open');
	}, () => {
